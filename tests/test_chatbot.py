...
class TestIntentClassifier:
    ...
    def test_restaurant_intent_arabic(self, clf):
        intent, conf = clf.predict("عايز مطعم حلو في الفيوم")
        assert intent == "fayoum_restaurants"

    def test_restaurant_intent_english(self, clf):
        intent, conf = clf.predict("Fayoum restaurants list")
        assert intent == "fayoum_restaurants"

    def test_tourguide_intent_arabic(self, clf):
        intent, conf = clf.predict("عايز مرشد سياحي")
        assert intent == "fayoum_tourguides"

    def test_tourguide_intent_english(self, clf):
        intent, conf = clf.predict("tour guide contacts")
        assert intent == "fayoum_tourguides"
...
class TestResponseStructure:
    ...
    def test_restaurants_response_has_all_keys(self):
        mock_rests = [{"id": 4, "name": "مطعم النيل"}]
        resp = build_restaurants_response("ar", mock_rests)
        assert self.REQUIRED_KEYS.issubset(resp.keys())
        assert resp["type"] == "card"
        assert len(resp["data"]["items"]) == 1

    def test_tourguides_response_has_all_keys(self):
        mock_guides = [{"id": 2, "name": "أحمد علي"}]
        resp = build_tourguides_response("en", mock_guides)
        assert self.REQUIRED_KEYS.issubset(resp.keys())
        assert resp["type"] == "card"
    ...
    def test_restaurants_link_format(self):
        mock_rests = [{"id": 7, "name": "Test Rest"}]
        resp = build_restaurants_response("en", mock_rests)
        assert "voyagoo.runasp.net/Restaurants/7" in resp["data"]["items"][0]["link"]

    def test_tourguides_link_format(self):
        mock_guides = [{"id": 5, "name": "Test Guide"}]
        resp = build_tourguides_response("en", mock_guides)
        assert "voyagoo.runasp.net/TourGuides/5" in resp["data"]["items"][0]["link"]
